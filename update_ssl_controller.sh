...
sshpass -p "${CONTROLLER_PASS}" ssh ${CONTROLLER_USER}@${CONTROLLER_HOST} "if [ -s \"${KEYSTORE}.orig\" ]; then echo -n \"Backup of original keystore exists! Creating non-destructive backup as keystore.bak...\"; cp -n \"${KEYSTORE}\" \"${KEYSTORE}.bak\"; else echo -n \"no original keystore backup found. Creating backup as keystore.orig...\"; cp -n \"${KEYSTORE}\" \"${KEYSTORE}.orig\"; fi"
echo "done!"

# Backup original keys on CK
echo -n "Creating backups of cloudkey.key and cloudkey.crt on controller..."
sshpass -p "${CONTROLLER_PASS}" ssh ${CONTROLLER_USER}@${CONTROLLER_HOST} "for f in {cloudkey.key,cloudkey.crt}; do cp -n \"/etc/ssl/private/${f}\" \"/etc/ssl/private/${f}.bak\"; done"
echo "done!"
